    ...
    if form.validate_on_submit():
        ...
        if result.status_code == 200:
            ...
            if account['account_id'] != -1:
                return redirect(url_for('home_view', account_email=account_email))
            else:
                return render_template('login/login.html', form=form)
...
def register_account():
    ...
    if form.validate_on_submit():
        ...
        if result.status_code == 200:
            return redirect(url_for('home_view', account_email=form.email.data))
        else:
...
# ---------------- Home Page ---------------- #
# home page
@app.route('/home/<string:account_email>', methods=['GET'])
def home_view(account_email):
    ...
    if result.status_code == 200:
        ...
        return render_template('individual/home_page.html', form=form, account=account_info)
    else:
        return redirect('login_view')


# sending registration email
@app.route('/home/<string:account_email>', methods=['POST'])
